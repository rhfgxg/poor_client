...
自定义数据包
	成员：数据头(枚举：客户端链接日志，登录数据，注册信息，文件切片上传等)，json数据包（包含具体的登录账号密码，文件ID等附加信息），可选的文件数据（传递切片文件数据）
	自定义序列化和解析函数
...
7-17：发现上传的数据包和接收到的数据包大小不一致
	所以在数据包头加入一段数据：数据包大小
		...
		在逐步查看字节流大小的变化后，发现在添加 json数据 和 文件数据 时，分别会额外增加4字节，
